// ILI9486 SPI display generators (TFT_eSPI)

Blockly.Arduino['xiaobai_ili9486_init'] = function (block) {
    ...
    Blockly.Arduino.definitions_['define_tft_ili9486'] = 'TFT_eSPI tft = TFT_eSPI();';
    Blockly.Arduino.definitions_['define_tft_ili9486_u8g2'] = 'U8g2_for_TFT_eSPI u8g2;';
    Blockly.Arduino.definitions_['define_tft_ili9486_utf8_helpers'] =
        'uint8_t ili9486ChineseFontSize = 16;\n' +
        '\n' +
        'static uint8_t ili9486NormalizeChineseFontSize(int requestedSize) {\n' +
        ...
        '}\n' +
        '\n' +
        'static const uint8_t* ili9486GetChineseFontBySize(uint8_t size, uint8_t pageIndex) {\n' +
        '  size = ili9486NormalizeChineseFontSize(size);\n' +
        '  switch (size) {\n' +
        '    case 12:\n' +
        '      switch (pageIndex) {\n' +
        ...
        '}\n' +
        '\n' +
        'static uint32_t ili9486DecodeUtf8(const char *s, uint8_t *bytesUsed) {\n' +
        ...
        '}\n' +
        '\n' +
        'static const uint8_t* ili9486SelectChineseFontForGlyph(uint8_t size, uint16_t codepoint) {\n' +
        '  for (uint8_t i = 0; i < 3; ++i) {\n' +
        '    const uint8_t *font = ili9486GetChineseFontBySize(size, i);\n' +
        '    u8g2.setFont(font);\n' +
        '    if (u8g2_IsGlyph(&u8g2.u8g2, codepoint)) {\n' +
        '      return font;\n' +
        '    }\n' +
        '  }\n' +
        '  return ili9486GetChineseFontBySize(size, 0);\n' +
        '}\n' +
        '\n' +
        'void ili9486DrawUtf8(String text, int16_t x, int16_t y, uint16_t color) {\n' +
        ...
        '    uint8_t bytesUsed = 0;\n' +
        '    uint32_t codepoint32 = ili9486DecodeUtf8(ptr, &bytesUsed);\n' +
        '    uint16_t codepoint = codepoint32 > 0xFFFF ? 0x003F : (uint16_t)codepoint32;\n' +
        '    const uint8_t *font = ili9486SelectChineseFontForGlyph(ili9486ChineseFontSize, codepoint);\n' +
        '    u8g2.setFont(font);\n' +
...
Blockly.Arduino['xiaobai_ili9486_fill_screen'] = function (block) {
    var color = block.getFieldValue('COLOR');
    return 'tft.fillScreen(' + color + ');\n';
};

Blockly.Arduino['xiaobai_ili9486_draw_text'] = function (block) {
    ...
    var code = '';
    code += 'ili9486DrawUtf8(String(' + text + '), ' + x + ', ' + y + ', ' + color + ');\n';
    return code;
};

Blockly.Arduino['xiaobai_ili9486_set_chinese_font_size'] = function (block) {
    var size = block.getFieldValue('SIZE') || '16';
    return 'ili9486ChineseFontSize = ili9486NormalizeChineseFontSize(' + size + ');\n';
};

Blockly.Arduino['xiaobai_ili9486_draw_line'] = function (block) {
...
Blockly.Arduino['xiaobai_ili9486_fill_rect'] = function (block) {
    var x = block.getFieldValue('X');
    var y = block.getFieldValue('Y');
